...
import { X, Filter, List, Search } from 'lucide-react';
...
export default function MapDrawer({
    ...
    loadingResults, loadingFacets, searchQuery, onSearchChange, resultsResetKey
}) {
    ...
    }, [resultsResetKey, activeTab]);

    const maxVisible = useMemo(
        () => Math.min(visibleCount, organizations.length),
        [visibleCount, organizations.length]
    );

    useEffect(() => {
        const container = resultsContainerRef.current;
        if (!container || activeTab !== 'results') return undefined;
        ...
        const loadMoreIfNeeded = () => {
            ...
            if (distanceToBottom <= SCROLL_THRESHOLD_PX) {
                setVisibleCount((current) => {
                    if (current >= organizations.length) return current;
                    return Math.min(current + RESULTS_BATCH_SIZE, organizations.length);
                });
            }
        };

        loadMoreIfNeeded();
        container.addEventListener('scroll', loadMoreIfNeeded, { passive: true });
        window.addEventListener('resize', loadMoreIfNeeded);

        return () => {
            container.removeEventListener('scroll', loadMoreIfNeeded);
            window.removeEventListener('resize', loadMoreIfNeeded);
        };
    }, [activeTab, organizations.length, loadingResults]);
                    ...
                    {activeTab === 'filters' ? (
                        ...
                        <div className="h-full flex flex-col">
                            ...
                            <div
                                ...
                                style={{ backgroundColor: 'rgba(89, 89, 91, 0.02)', borderColor: 'rgba(89, 89, 91, 0.05)' }}
                            >
                                <ResultsList
